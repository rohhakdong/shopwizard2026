...
public class ByteUtil {
    ...
    public static int toInt(byte[] src, int srcPos) {
        int dword = 0;
        for (int i = 0; i < 4; i++) dword = (dword << 8) + (src[i + srcPos] & 0xFF);
        return dword;
    }
    ...
    public static long toLong(byte[] src, int srcPos) {
        long qword = 0;
        for (int i = 0; i < 8; i++) qword = (qword << 8) + (src[i + srcPos] & 0xFF);
        return qword;
    }
    ...
    public static void toBytes(int value, byte[] dest, int destPos) {
        for (int i = 0; i < 4; i++) dest[i + destPos] = (byte)(value >> ((7 - i) * 8));
    }
    ...
    public static void toBytes(long value, byte[] dest, int destPos) {
        for (int i = 0; i < 8; i++) dest[i + destPos] = (byte)(value >> ((7 - i) * 8));
    }
    ...
    public static String toHexString(byte[] bytes) {
        ...
        for (byte byt : bytes) {
            result.append(Integer.toString((byt & 0xF0) >> 4, 16));
            result.append(Integer.toString(byt & 0x0F, 16));
        }
        return result.toString();
    }
